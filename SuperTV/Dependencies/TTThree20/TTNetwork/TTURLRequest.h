...
// Core
#import "SHThree20Extral.h" // For __TTDEPRECATED_METHOD
...
@interface TTURLRequest : NSObject {
  ...
  BOOL  _filterPasswordLogging;
    ...
    BOOL    _isBreakSeqment;

  NSMutableArray* _delegates;
    
    URLRequestType requestType; 
    TTURLCache * _requestCache;
}
 ...
 * 代表是否支持断点续传标示.
 */
@property (nonatomic) BOOL breakSeqment;

/**
 * 断点续传片段文件信息.
 */
@property (nonatomic,retain) NSDictionary* seqmentData;
...
/**
 * 区分Request的类型：文本 or 图片
 */
@property (nonatomic, assign) URLRequestType requestType;
@property (nonatomic, assign) TTURLCache *requestCache;
