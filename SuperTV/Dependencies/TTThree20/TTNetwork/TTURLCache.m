...
static const  CGFloat   kLargeImageSize = 600 * 450;

static        NSString* kDefaultCacheName       = @"sohunews2";
...
- (NSString*)cachePathForKey:(NSString*)key {
    ...
    if (self.mainKey != nil)
    {
        NSString* strKey = [_cachePath stringByAppendingPathComponent:self.mainKey];
        
        [TTURLCache createPathIfNecessary:strKey];
        
        return [strKey stringByAppendingPathComponent:key];
    }
...
- (NSData*)dataForURL:(NSString*)URL {
    if ([TTURLCache sharedImgsCache] == self) {
        return [self dataForURL:URL expires:TT_CACHE_EXPIRATION_AGE_NEVER timestamp:nil];
    }
  return [self dataForURL:URL expires:0 timestamp:nil]; // edited by TengSong on 20120214  : TT_CACHE_EXPIRATION_AGE_NEVER -> 0 默认缓存永久有效
}
...
- (void) storeSegment:(long long)totalLength current:(long long)currentLength data:(NSData*)cacheData key:(NSString*)strKey
{
    NSString* strDir = [self cachePathForKey:strKey];
    
    NSString* strPath = [strDir stringByAppendingPathExtension:@"cah"];
    
    NSFileManager* fm = [NSFileManager defaultManager];
    
    if ([fm fileExistsAtPath:strDir])
    {
        NSFileHandle* fileHandle = [NSFileHandle fileHandleForWritingAtPath:strDir];
    ...
    {
        [fm createFileAtPath:strDir contents:cacheData attributes:nil];
    }
    
    NSArray* argValue = [NSArray arrayWithObjects:
                         [NSString stringWithFormat:@"%qi",totalLength],[NSString stringWithFormat:@"%qi",currentLength], nil];
    
    NSArray* argKey = [NSArray arrayWithObjects:kSeqmentTotalLength,kSeqmentCurrentLength, nil];
    
    NSDictionary* dict = [NSDictionary dictionaryWithObjects:argValue forKeys:argKey];
    
    [fm removeItemAtPath:strPath error:nil];
    [dict writeToFile:strPath atomically:YES];
}

- (void) finishSegment:(NSData*)cacheData key:(NSString*)strKey
{
    NSString* strDir = [self cachePathForKey:strKey];
...
- (NSDictionary*) seqmentForKey:(NSString*)strKey
{
    NSString* strDir = [self cachePathForKey:strKey];
    
    NSString* strPath = [strDir stringByAppendingPathExtension:@"cah"];
    
    return [NSDictionary dictionaryWithContentsOfFile:strPath];
}

- (NSDictionary*) seqmentForUrl:(NSString*)strUrl
{
    return [self seqmentForKey:[self keyForURL:strUrl]];
}


/*
 edited by tengsong at 20110711
 */
+ (TTURLCache *)sharedDocsCache
{
    return [SHThree20Extral sharedDocsCache];
}
