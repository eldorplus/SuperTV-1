...
- (CGRect)frameOfPageAtIndex:(NSInteger)pageIndex {
  CGSize size;
  if (_dataSource && [_dataSource respondsToSelector:@selector(scrollView:sizeOfPageAtIndex:)]) {
...
- (void)updateZooming:(UIEdgeInsets)edges {
  if (!_zooming && (self.zoomed || [self edgesAreZoomed:edges])) {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(scrollViewDidBeginZooming:)]) {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(scrollViewDidEndZooming:)]) {
...
- (void)rotationDidStop {
  if (_delegate && [_delegate respondsToSelector:@selector(scrollViewDidRotate:)]) {
...
- (void)tapTimer:(NSTimer*)timer {
  ...
  if (_delegate && [_delegate respondsToSelector:@selector(scrollView:tapped:)]) {
...
- (void)beginHolding {
  ...
  if (_delegate && [_delegate respondsToSelector:@selector(scrollViewDidBeginHolding:)]) {
...
- (void)endHolding {
  ...
  if (_delegate && [_delegate respondsToSelector:@selector(scrollViewDidEndHolding:)]) {
...
- (void)touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event {
  ...
  if (_touchCount < 2) {
    ...
    for (UITouch* touch in touches) {
      ...
      if (_touchCount == 1) {
        if (_delegate && [_delegate respondsToSelector:@selector(scrollView:touchedDown:)]) {
    ...
    if (touch == _touch1 || touch == _touch2) {
      ...
      if (_touchCount == 1) {
        ...
        if (touch.tapCount == 1 && !_dragging) {
          _executingZoomGesture = NO;
          if (_delegate && [_delegate respondsToSelector:@selector(scrollView:touchedUpInside:)]) {
...
- (void)setOrientation:(UIInterfaceOrientation)orientation animated:(BOOL)animated {
  if (orientation != _orientation && [self supportsOrientation:orientation]) {
    if (_delegate && [_delegate respondsToSelector:@selector(scrollViewWillRotate:toOrientation:)]) {
...
@implementation TTScrollView(extral)
