...
- (NSRange)isUrl:(NSString*)strUrl start:(NSRange)startRange
{
	NSRange range = NSMakeRange(NSNotFound, 0);
	for (int i=startRange.location+7; i<strUrl.length; i++)
	{
		unichar c = [strUrl characterAtIndex:i];
...
- (void)parseURLs:(NSString*)string {
  ...
  while (index < string.length) {
    ...
    if (startRange.location == NSNotFound) {
      ...
      //                           range:searchRange];
		NSRange endRange = [self isUrl:string start:searchRange];
