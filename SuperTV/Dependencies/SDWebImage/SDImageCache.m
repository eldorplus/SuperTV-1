...
- (NSString *)cachePathForKey:(NSString *)key
{
    ...
    //NSString *result = [diskCachePath stringByAppendingPathComponent:filename];
...
- (void)storeKeyWithDataToDisk:(NSArray *)keyAndData
{
    ...
    NSData *data = [keyAndData count] > 2 ? [keyAndData objectAtIndex:1] : nil;
    BOOL isNotification = [keyAndData count] > 2 ? YES : NO;
//    TTDPRINT(@"\n\n=========\n%@\n=============\n\n", key);
    if (data)
    {
//        [fileManager createFileAtPath:[self cachePathForKey:key] contents:data attributes:nil];
        // edit by malei
        BOOL result = [fileManager createFileAtPath:[self cachePathForKey:key] contents:data attributes:nil];
        if (result && isNotification) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"downloadImageFinishNews" object:key];
            });
        }
    }
    else
...
#if TARGET_OS_IPHONE
            // edit by malei
            BOOL result = [fileManager createFileAtPath:[self cachePathForKey:key] contents:UIImageJPEGRepresentation(image, (CGFloat)1.0) attributes:nil];
            if (result && isNotification) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"downloadImageFinishNews" object:key];
                });
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk isNewsContent:(BOOL)newsContent
{
    if (!image || !key)
    {
        return;
    ...
    if (toDisk)
    {
        ...
        if (data)
        {
            keyWithData = newsContent ? [NSArray arrayWithObjects:key, data, @"1", nil] : [NSArray arrayWithObjects:key, data, nil];
        }
        else
        {
            keyWithData = newsContent ? [NSArray arrayWithObjects:key, @"1", nil] : [NSArray arrayWithObjects:key, nil];
        }
...
{
    [self storeImage:image imageData:data forKey:key toDisk:toDisk isNewsContent:NO];
}
