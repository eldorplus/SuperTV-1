...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    // Add by Chen Zhiqiang 2013.9.13
    if (![self canDownload])
    {
        return;
    }
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    if (image)
    {
        ...
        if (userInfo && ![userInfo isKindOfClass:[NSNull class]] && [userInfo objectForKey:@"downloadNotification"]
            && [[userInfo objectForKey:@"downloadNotification"] intValue]) {
...
- (BOOL)canDownload {
    ...
    Reachability *reachability = appDelegate.reachability;
    BOOL bWAN = [reachability isReachableViaWWAN];
    
    NSString *picMode = [[NSUserDefaults standardUserDefaults] objectForKey:kNonePictureModeKey];
    BOOL bSelected = [picMode intValue] == kPicModeAlways ? NO : YES;
    if (bWAN && bSelected) {
        return NO;
    }
    */
